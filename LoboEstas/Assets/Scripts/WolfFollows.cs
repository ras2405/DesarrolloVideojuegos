        ...
        if (cycleDayController != null && cycleDayController.IsNight() && !cycleDayController.IsAfter10PM())
        {
            if (player != null)
            {
                    ...
                    player.position.y >= -11f && player.position.y <= 11f)
                {
                    ...
                    {
                        // Cambiar la animación en función de la distancia
                        float distanceToPlayer = Vector3.Distance(transform.position, player.position);
                        if (distanceToPlayer < transitionDistance)
                        {
                            // El lobo está cerca, activa la animación caminata2
                            animator.SetBool("isNear", true);
                        }
                        else
                        {
                            // El lobo está lejos, vuelve a la animación caminata1
...
public class FollowPlayer : MonoBehaviour
{
    ...
    private bool isWolfActive = false;
    private bool isWolfReady = false; // Nuevo indicador para saber si el lobo está listo para aparecer
    ...
    void Update()
    {
        ...
        if (cycleDayController != null && cycleDayController.IsNight() && !cycleDayController.IsAfter10PM())
        {
            // Si el lobo no está activo y es de noche, activarlo
            if (!isWolfActive && !isWolfReady)
            {
                // Hacer que el lobo esté listo para aparecer
                isWolfReady = true;
                ActivateWolf();
            }
    ...
    void ActivateWolf()
    {
        // Cuando el lobo está listo para aparecer, activar todo lo necesario
        spriteRenderer.enabled = true;
        animator.enabled = true;
        wolfAudioSource.enabled = true;

        // Aquí puedes poner más lógicas para que el lobo se mueva hacia el jugador o realice alguna animación de activación
...
public class FollowPlayer : MonoBehaviour
{
    ...
    public float captureDistance = 0.5f; // Distancia mínima para atrapar al jugador
    public float transitionDistance = 2.0f; // Distancia para cambiar entre caminata1 y caminata2
    public float reducedSpeed = 0.4f; // Velocidad reducida cuando el lobo está cerca del jugador
        ...
        if (cycleDayController != null && cycleDayController.IsNight() && !cycleDayController.IsAfter10PM())
        {
            ...
            if (isWolfActive && player != null)
            {
                    ...
                    player.position.y >= -11f && player.position.y <= 11f)
                {
                    ...
                    // Reduce la velocidad si el lobo está cerca del jugador (en la animación caminata2)
                    float currentSpeed = followSpeed;
                    if (distanceToPlayer < transitionDistance)
                    {
                        currentSpeed = reducedSpeed;
                        animator.SetBool("isNear", true); // Cambia la animación cuando está cerca
                    }
                    else
                    {
                        animator.SetBool("isNear", false); // Vuelve a la animación caminata1
                    }
                    ...
                    // Guarda la posición actual para la próxima comparación
                    lastPosition = transform.position;
...
public class FollowPlayer : MonoBehaviour
{
    ...
    public AudioSource branchBreakSound;  // Sonido de rama quebrándose
        // Sonido de aullido

    // Para controlar el momento de la activación de la rama
    ...
    void Update()
    {
        ...
        if (cycleDayController != null && cycleDayController.IsNight() && !cycleDayController.IsAfter10PM())
        {
            ...
            if (!isWolfActive && !isWolfReady)
            {
                ...
                // Reproducir el sonido de la rama quebrándose solo una vez antes de la activación
    ...
    {
        // Cuando el lobo está listo para aparecer, activar todo lo necesario
        // Activamos el SpriteRenderer, Animator y AudioSource solo cuando el lobo está por aparecer
        spriteRenderer.enabled = true;
        animator.enabled = true;
        wolfAudioSource.enabled = true;
